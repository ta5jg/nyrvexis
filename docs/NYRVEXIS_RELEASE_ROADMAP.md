<!-- =============================================================================
File:           docs/NYRVEXIS_RELEASE_ROADMAP.md
...
Description:
  Nyrvexis → yayına kadar fazlı plan (web + Capacitor mobil önce; ayrı native sonra).
  ...
  v1.13: R9 başlangıcı — parity SSOT (`docs/FULL_PRODUCT_PARITY_ROADMAP.md`); combat tempo (`battlePacing.actThreshold`) + web Auto-play süresi + arena VFX (sway / impact ripple).
  v1.14: R9/R10 tabloları — web arena doktrini + Unity altın sahne köprüsü (`pnpm run unity:golden-export`); `FULL_PRODUCT_PARITY_ROADMAP` R10–R13 yeniden numaralandırıldı.
  v1.14.1: R10.P7 — `KrBattleEvent.presentation` (anim niyeti SSOT); gateway sim + Unity `PlayFromIntent`.

License:
  Proprietary. All rights reserved. See LICENSE in the repository root.
============================================================================= -->

# Nyrvexis — Yayın Yol Haritası (SSOT)

**Öncelik:** Nyrvexis oyunu.  
**Mimari:** Tek repo; **Web** (`companion-web`) + **Mobil** (`companion-mobile` / Capacitor). Sunucu **tek doğruluk kaynağı** (`services/gateway`).  
**Sunum:** Varsayılan gönderim **Canvas arena** (`ArenaCanvas`). **Unity** paketi (`apps/game-unity/UnityPackage`) aynı JSON ile **altın sahne** ve Animator omurgası üretir; simülasyon SSOT gateway’de kalır.  
**İkinci aşama:** İsteğe bağlı ayrı native veya Unity/WebGL istemci — aynı `@nyrvexis/protocol`.

**Son güncelleme notları:** final teslimat sırası için **`docs/NYRVEXIS_FINAL_SHIP_STEPS.md`**.
...
- Görsel/VFX minimum kabul çubuğu: arena **operasyonel olarak okunur** (hareket / manevra / vuruş geri bildirimi); statik sıra auto-battler çubuğu anti-hedef. Web’de `ArenaCanvas`; premium çizgi için Unity Animator borusu (`docs/FULL_PRODUCT_PARITY_ROADMAP.md` R10).
...
| R1.3 | iOS/Android iç test build | **Tamam:** CI `nyrvexis-ci.yml` web derlemesi + gateway typecheck; mağaza yükleme Xcode/Android Studio ile yerel |
...
| ID | Çıktı | Durum |
|----|--------|--------|
| R5.1 | Art direction + birim sprite setleri | **Tamam:** `visual/artDirection.ts` (`NYRVEXIS_ART` tema `nyrvexa_twilight_v1`, rol hex SSOT); mevcut sheet crop + `spriteProfiles` ile uyumlu not |
...
| R5.4 | Ses paketi | **Tamam:** `public/audio/kr_*.wav` (22 kHz PCM, `pnpm gen:sfx` ile üretilir) → `decodeAudioData`; yükleme `preloadSfxSamples` / ilk turdan önce `queueMicrotask` prefetch; hazır değilse osilatör yolu; `nyrvexis_sfx_muted` + **Sound** anahtarı |
...
| ID | Çıktı | Durum |
|----|--------|--------|
| R6.1 | E2E smoke + CI gate | **Tamam:** Playwright `apps/companion-web/e2e/smoke.spec.ts` (gate→dashboard, Run battle→Result); `scripts/ci-e2e-smoke.sh`; `nyrvexis-ci.yml` Chromium + `--with-deps`; yerel `pnpm --filter @nyrvexis/companion-web run test:e2e` (sunucular script ile veya elle). Onboarding overlay: test `kr_onboarding_v1_done` init |
...
## Faz R9 — Combat spectacle + doktrin (web öncelik)

Parite SSOT: **`docs/FULL_PRODUCT_PARITY_ROADMAP.md`** § R9.

| ID | Çıktı | Durum |
|----|--------|--------|
| R9.1 | Sim tempo + replay sıklığı (`battlePacing.actThreshold`; Auto-play hedef süresi) | **Tamam** |
| R9.2 | Arena juice: sway, impact ripple, shake / mermi hattı (`ArenaCanvas`) | **Tamam** |
| R9.3 | Manevra olayları okunur sahada (take_cover / probe / bound → görsel zarf) | **Tamam** (gateway sim + Canvas agregasyonu) |

---

## Faz R10 — Altın sahne (Unity sunum katmanı)

Parite SSOT: **`docs/FULL_PRODUCT_PARITY_ROADMAP.md`** § R10. Özet komut: **`pnpm run unity:golden-export`** → `Resources/nyrvexis-golden-battle-export.json`. Unity Editor kurulum (TR): **`apps/game-unity/UNITY_SETUP.md`**.

| ID | Çıktı | Durum |
|----|--------|--------|
| R10.1 | Protokol DTO + export şekli (`request` + `result`) — `KrBattleModels.cs` | **Tamam** |
| R10.2 | Replay indeks + slot layout web ile hizalı — `BattleReplayIndex`, `ArenaLayout` | **Tamam** |
| R10.3 | **Idle / Advance / Attack / Hit / Death** — `NyrvexisReplayDirector` + `NyrvexisAnimStates` | **Tamam** (programatik clip yer tutucu → sanat rig bekleniyor) |
| R10.4 | Golden JSON örneği + export script | **Tamam** |
| R10.5 | Rigged karakter + gerçek animasyon clip’leri | **Yapılacak** |
| R10.6 | Companion’dan tek tık Unity export indirme | **Tamam:** battle sonrası **Export Unity JSON** (`App.tsx`); `NyrvexisBattleExportDto` |
| R10.7 | Sim’e isteğe bağlı görsel-only alanlar (pozisyon / anim niyeti) | **Tamam:** `KrBattleEvent.presentation` (`srcIntent` / `dstIntent`); gateway `battleSim.ts`; Unity `PlayFromIntent` |

---

## Faz N (sonra) — Ayrı native / Unity dağıtımı

Backend ve `@nyrvexis/protocol` sabit; istemci tarafında öncelik **sunum + platform API**. **R10** repo içinde tutulur; **Faz N**, Unity/WebGL veya tamamen ayrı ikili için mağaza, güncelleme kanalı ve minimum sürüm politikası anlamına gelir (web + Capacitor ile paralel veya sonradan).
...
# Unity altın sahne için battle export (çalışan gateway + detay: apps/game-unity/README.md)
