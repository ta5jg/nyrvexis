...
- [ ] **Yasal URL’ler:** Gizlilik / şartlar / destek / hesap-veri — gateway **`KR_LEGAL_*`** + companion **`GET /legal/public`** (`docs/NYRVEXIS_RELEASE_ROADMAP.md` R7).
...
- [ ] **Migrasyon:** `pnpm --filter @nyrvexis/gateway db:migrate` veya container girişindeki `psql` sırası (`deploy/docker-entrypoint-gateway.sh`).
